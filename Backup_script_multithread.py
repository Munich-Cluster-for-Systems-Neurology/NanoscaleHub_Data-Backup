...
with open(wg_file, 'r') as file:
    ...
    print("Loaded working groups: ", working_groups)
...
with open(surname_file, 'r') as file:
    ...
    print("Loaded surname to working group mappings.")
...
with open(path_file, 'r') as file:
    ...
    print(f"Loaded paths: Source - {paths['source_path']}, Backup - {paths['backup_path']}")
...
with open(date_range_file, 'r') as file:
    ...
    print(f"Loaded date range: Start - {date_config['start_date'].date()}, End - {date_config['end_date'].date()}")
...
def copy_file(source, destination_dir):
    ...
    if not os.path.exists(destination_file):
        ...
        return f"File {os.path.basename(source)} copied to {destination_dir}"
    else:
        return f"File {os.path.basename(source)} already exists at {destination_dir}, not copied."

def backup_files(source_dir, backup_root):
    files_to_copy = []
    print(f"Starting file search in {source_dir}...")
    with ThreadPoolExecutor(max_workers=64) as executor:  # Adjust the number of workers as needed
                ...
                if any(file.endswith(ext) for ext in extensions):
                    ...
                    if start_date <= file_mtime <= end_date:
                        ...
                        if wg and surname and ms_folder:
                            ...
                            files_to_copy.append((file_path, destination_dir))
                            print(f"File {file} scheduled for backup.")

        # Now, map the copying tasks
        copy_results = [executor.submit(copy_file, file_path, dest_dir) for file_path, dest_dir in files_to_copy]
        for future in as_completed(copy_results):
            print(future.result())
